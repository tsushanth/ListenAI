...
final class AuthService: NSObject, ObservableObject {
    ...
    private let googleClientID = "85846747177-6dr3b825m6aj4tavspc21real3rdpuba.apps.googleusercontent.com"
    private let googleRedirectURI = "com.googleusercontent.apps.85846747177-6dr3b825m6aj4tavspc21real3rdpuba:/oauth2redirect"
    private var googleAuthSession: ASWebAuthenticationSession?

    /// Sign in with Google.
    /// Uses OAuth 2.0 with PKCE to get an ID token for Supabase Auth.
    func signInWithGoogle() async throws {
        ...
        // Build authorization URL - request ID token via OpenID Connect
        ...
        components.queryItems = [
            ...
            URLQueryItem(name: "nonce", value: randomNonceString())
        ]

        guard let authURL = components.url else {
            throw AuthError.googleSignInFailed("Invalid authorization URL")
        }
        ...
        let callbackURL = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<URL, Error>) in
            let session = ASWebAuthenticationSession(
                url: authURL,
                callbackURLScheme: "com.googleusercontent.apps.85846747177-6dr3b825m6aj4tavspc21real3rdpuba"
        ...
        guard let components = URLComponents(url: callbackURL, resolvingAgainstBaseURL: false),
            ...
            throw AuthError.googleSignInFailed("No authorization code received")
        }

        // Exchange code for tokens (including ID token)
        let (idToken, accessToken) = try await exchangeGoogleCodeForTokens(code: code, verifier: verifier)

        // Exchange with backend for Supabase session
        ...
        print("[AuthService] Google Sign In successful for \(user?.email ?? "unknown")")
    }

    /// Exchange Google authorization code for tokens
    private func exchangeGoogleCodeForTokens(code: String, verifier: String) async throws -> (idToken: String, accessToken: String) {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            print("[AuthService] Google token exchange failed: \(errorBody)")
            throw AuthError.googleSignInFailed("Token exchange failed")
        }
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw AuthError.googleSignInFailed("Invalid token response")
        }

        return (idToken, accessToken)
    }

    // MARK: - PKCE Helpers for Google
    ...
    private func exchangeGoogleToken(
        ...
        var request = URLRequest(url: backendURL.appendingPathComponent("/api/auth/google"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue(getDeviceId(), forHTTPHeaderField: "X-Device-ID")
