...
enum UsageTier: String, Codable, CaseIterable, Sendable {
    ...
    var dailyLimit: Int {
        switch self {
        case .free: return 4_000           // ~5 minutes of audio
    ...
    var monthlyLimit: Int {
        switch self {
        case .free: return 25_000          // ~30 minutes total per month
...
// MARK: - Premium Usage Summary

/// Summary of premium quality (ElevenLabs) usage for the day.
struct PremiumUsageSummary: Sendable {
    let samplesUsed: Int
    let samplesLimit: Int
    let charactersUsed: Int
    let charactersLimit: Int
    let canUsePremium: Bool
    let tier: UsageTier

    var samplesRemaining: Int {
        if samplesLimit == Int.max { return Int.max }
        return max(0, samplesLimit - samplesUsed)
    }

    var charactersRemaining: Int {
        if charactersLimit == Int.max { return Int.max }
        return max(0, charactersLimit - charactersUsed)
    }

    var usagePercentage: Float {
        if samplesLimit == Int.max { return 0 }
        return Float(samplesUsed) / Float(samplesLimit)
    }
    ...
    var formattedSamplesRemaining: String {
        if samplesRemaining == Int.max { return "Unlimited" }
        return "\(samplesRemaining) of \(samplesLimit)"
    }

    var formattedCharactersRemaining: String {
        if charactersRemaining == Int.max { return "Unlimited" }
        return formatCharacterCount(charactersRemaining)
    }

    var statusMessage: String {
        if tier == .unlimited {
            return "Unlimited premium quality"
        }
        if isExhausted {
            return "Premium samples used for today. Resets at midnight."
        }
        if samplesRemaining == 1 {
            return "1 premium sample left today"
        }
        return "\(samplesRemaining) premium samples left today"
    }
}
