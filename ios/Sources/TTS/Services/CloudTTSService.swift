...
struct CloudTTSConfiguration: Sendable {
    ...
    // NOTE: ElevenLabs API key should be configured server-side via backend.
    // Direct API calls should go through the backend to keep keys secure.
    static let elevenLabs = CloudTTSConfiguration(
        apiKey: nil,  // API key removed - use backend for ElevenLabs synthesis
    ...
    /// Self-hosted TTS service (Coqui XTTS v2 on GCP)
    /// URL should be set via environment/config when available
    static let selfhosted = CloudTTSConfiguration(
        apiKey: nil, // Optional API key for authentication
        baseURL: URL(string: "https://tts.listenai.app")!, // Default, overridable
        maxChunkSize: 10000, // Self-hosted can handle larger chunks
        maxConcurrentRequests: 3,
        timeoutInterval: 120, // Longer timeout for GPU inference
        retryCount: 3,
        retryDelay: 2.0
    )

    static func configuration(for provider: VoiceProvider) throws -> CloudTTSConfiguration {
        switch provider {
        case .apple:
            throw CloudTTSError.unsupportedProvider("Apple provider should use OnDeviceTTSService")
        case .elevenLabs:
            return .elevenLabs
        case .openAI:
            return .openAI
...
actor CloudTTSService: TTSService {
    ...
    private func buildValidationRequest(apiKey: String) -> URLRequest {
        ...
        case .selfhosted:
            request = URLRequest(url: configuration.baseURL.appendingPathComponent("health"))
            if !apiKey.isEmpty {
                request.setValue(apiKey, forHTTPHeaderField: "X-API-Key")
            }
    ...
    private func synthesizeChunk(
        ...
        for attempt in 0..<configuration.retryCount {
            do {
                ...
                case 401:
                    // Check if this is actually a rate limit error disguised as 401
                    // ElevenLabs sometimes returns 401 with rate limit message
                ...
                case 403:
                    ...
                    let message = String(data: data, encoding: .utf8) ?? ""
                    if message.lowercased().contains("rate") || message.lowercased().contains("limit") || message.lowercased().contains("exceeded") {
                        throw TTSError.rateLimited(retryAfter: 60)
                    }
                    throw TTSError.apiError(
                        provider: provider.displayName,
                ...
                default:
                    ...
                    // Check for rate limit keywords in the response
                    if message.lowercased().contains("rate") || message.lowercased().contains("limit") || message.lowercased().contains("too many") {
                        throw TTSError.rateLimited(retryAfter: 60)
                    }
    ...
    private func buildSynthesisRequest(
        ...
        case .selfhosted:
            request = try buildSelfHostedRequest(text: text, voice: voice, options: options, apiKey: apiKey)
    ...
    /// Build request for self-hosted TTS service (Coqui XTTS v2)
    private func buildSelfHostedRequest(
        text: String,
        voice: VoicePreset,
        options: SynthesisOptions,
        apiKey: String
    ) throws -> URLRequest {
        let url = configuration.baseURL.appendingPathComponent("synthesize")
        ...
        // Map voice preset to self-hosted voice name
        // Self-hosted service uses simpler voice identifiers
        let selfHostedVoiceId = mapToSelfHostedVoice(voice.providerVoiceID)

        let body: [String: Any] = [
            "text": text,
            "voice": selfHostedVoiceId,
            "language": voice.language.components(separatedBy: "-").first ?? "en",
            "speed": options.speed,
            "temperature": 0.7,
    ...
    /// Map cloud provider voice IDs to self-hosted voice names
    private func mapToSelfHostedVoice(_ cloudVoiceId: String) -> String {
        // Map popular ElevenLabs voices to self-hosted equivalents
        let voiceMapping: [String: String] = [
            "21m00Tcm4TlvDq8ikWAM": "rachel",      // Rachel (Warm & Clear)
            "pNInz6obpgDQGcFmaJgB": "adam",        // Adam (Professional)
            "nPczCjzI2devNBz1zQrb": "brian",       // Brian (Deep British)
            "EXAVITQu4vr4xnSDxMaL": "bella",       // Bella (Calm & Soothing)
            "TxGEqnHWrfWFTfGW9XjX": "josh",        // Josh (Storyteller)
            "XB0fDUnXU5powFXDhCwa": "charlotte",   // Charlotte (Elegant)
        ]

        return voiceMapping[cloudVoiceId] ?? "rachel"  // Default to rachel
    }
        ...
        case .selfhosted:
            return [
                VoicePreset(
                    ...
                    gender: .female,
                    style: .conversational,
                    voiceDescription: "Warm and clear female voice",
                    tier: .free
                ),
                VoicePreset(
                    name: "Adam",
                    provider: .selfhosted,
                    providerVoiceID: "adam",
                    gender: .male,
                    style: .narrative,
                    voiceDescription: "Professional male narrator voice",
                    tier: .free
                ),
                VoicePreset(
                    name: "Brian",
                    provider: .selfhosted,
                    providerVoiceID: "brian",
                    gender: .male,
                    style: .narrative,
                    voiceDescription: "Deep British male voice",
                    tier: .free
                ),
                VoicePreset(
                    name: "Bella",
                    provider: .selfhosted,
                    providerVoiceID: "bella",
                    gender: .female,
                    style: .calm,
                    voiceDescription: "Calm and soothing female voice",
                    tier: .free
                ),
                VoicePreset(
                    name: "Josh",
                    provider: .selfhosted,
                    providerVoiceID: "josh",
                    gender: .male,
                    style: .dramatic,
                    voiceDescription: "Storyteller voice for audiobooks",
                    tier: .free
                ),
                VoicePreset(
                    name: "Charlotte",
                    provider: .selfhosted,
                    providerVoiceID: "charlotte",
                    gender: .female,
                    style: .conversational,
                    voiceDescription: "Elegant and articulate female voice",
                    tier: .free
                )
            ]
