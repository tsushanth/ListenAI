...
actor PDFImportService {
    ...
    private func extractFromDocument(
        ...
        var combinedText = allText.joined(separator: "\n\n")
        var usedOCR = false

        // Check if we got any text - if not, try OCR
        if combinedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
           combinedText.split(separator: " ").count < 20 {
            // Try OCR for scanned/image-based PDFs
            do {
                let ocrResult = try await OCRService.shared.extractText(from: document, sourceURL: sourceURL)
                if !ocrResult.isEmpty {
                    combinedText = ocrResult.text
                    usedOCR = true
                    warnings.append(ExtractionWarning(
                        type: .lowConfidence,
                        message: "Text extracted using OCR (image recognition)",
                        suggestion: "OCR confidence: \(ocrResult.confidencePercentage)%. Review for accuracy."
                    ))
                } else {
                    throw ExtractionError.pdfNoText
                }
            } catch {
                // If OCR also fails, throw the original error
                throw ExtractionError.pdfNoText
            }
        }
        ...
        var confidence: Float = min(1.0, Float(charsPerPage) / 1000.0)
        ...
        let extractorName = usedOCR ? "PDFImportService+OCR" : "PDFImportService"
