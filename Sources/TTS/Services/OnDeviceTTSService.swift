...
actor OnDeviceTTSService: TTSService {
    ...
    func isVoiceAvailable(_ voice: VoicePreset) async -> Bool {
        ...
        return availableVoices.contains { $0.identifier == voice.providerVoiceID }
    }
    ...
    private func synthesizeSection(
        ...
        // Get the AVSpeechSynthesisVoice
        guard let avVoice = AVSpeechSynthesisVoice(identifier: voice.providerVoiceID) else {
            throw TTSError.voiceNotAvailable(voiceName: voice.name)
        }
        ...
        utterance.rate = mapSpeedToAVSpeechRate(options.speed * voice.defaultSpeed)
        utterance.pitchMultiplier = options.pitch * voice.defaultPitch
        utterance.volume = options.volume * voice.defaultVolume

        // Use word boundary for natural speech
        utterance.prefersAssistiveTechnologySettings = false

        // Synthesize using the write API (iOS 13+)
        return try await withCheckedThrowingContinuation { continuation in
            ...
            let delegate = WordTrackingDelegate { range in
                let word = (section.text as NSString).substring(with: range)
                // Estimate timing based on position (will be refined)
                let progress = Double(range.location) / Double(section.text.count)
                let estimatedDuration = Double(section.text.count) / 150.0 * 60.0 / Double(options.speed)

                let wordTimestamp = WordTimestamp(
                    word: word,
                    startTime: progress * estimatedDuration,
                    endTime: progress * estimatedDuration + 0.1,
                    sectionIndex: section.index,
                    characterRange: range.location..<(range.location + range.length)
                )
            ...
            // Use the write API to capture audio
            synth.write(utterance) { buffer in
                guard let pcmBuffer = buffer as? AVAudioPCMBuffer else {
                    ...
                    // Writing complete, merge buffers and save
                    do {
                        let result = try self.saveBuffersToFile(
                            audioBuffers,
                            to: outputURL
                        )
                        continuation.resume(returning: (result.url, result.duration, wordTimestamps))
                    } catch {
    ...
    private nonisolated func convertAudioFile(_ url: URL, to format: AudioFormat) async throws -> URL {
        ...
        case .failed:
            throw TTSError.audioEncodingFailed(
                reason: exportSession.error?.localizedDescription ?? "Conversion failed"
            )
        case .cancelled:
            throw TTSError.cancelled
        default:
            throw TTSError.audioEncodingFailed(reason: "Unknown conversion status")
        }
    }
