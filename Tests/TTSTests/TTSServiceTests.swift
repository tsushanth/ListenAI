...
final class TTSServiceTests: XCTestCase {
    ...
    func testCharacterVoicePresets() {
        let santa = VoicePreset.santa

        XCTAssertEqual(santa.name, "Santa Claus")
        XCTAssertTrue(santa.isCharacterVoice)
        XCTAssertEqual(santa.characterID, "santa")
        XCTAssertEqual(santa.tier, .premium)
        XCTAssertEqual(santa.emotion, .happy)
    }

    func testAllCharacterVoices() {
        let characters = VoicePreset.allCharacterVoices
