...
final class VoicePresetTests: XCTestCase {
    ...
    func testSantaStoryteller() {
        let preset = VoicePreset.santaStoryteller

        XCTAssertEqual(preset.name, "Santa Storyteller")
        XCTAssertTrue(preset.isCharacterVoice)
        XCTAssertEqual(preset.category, .character)
        XCTAssertEqual(preset.tier, .premium)
        XCTAssertEqual(preset.emotion, .cheerful)
        XCTAssertEqual(preset.age, .senior)
        XCTAssertNotNil(preset.characterID)
    }

    func testEnergeticYouTuber() {
        let preset = VoicePreset.energeticYouTuber

        XCTAssertEqual(preset.name, "Energetic YouTuber")
        XCTAssertEqual(preset.category, .podcast)
        XCTAssertEqual(preset.style, .excited)
        XCTAssertEqual(preset.tier, .premium)
        XCTAssertGreaterThan(preset.styleParameters.speakingRate, 1.0)
        XCTAssertGreaterThan(preset.styleParameters.expressiveness, 0.8)
    }
    ...
    func testAllBuiltInPresets() {
        ...
        XCTAssertTrue(presets.contains(where: { $0.name == "Narrator" }))
        XCTAssertTrue(presets.contains(where: { $0.name == "Calm Teacher" }))
        XCTAssertTrue(presets.contains(where: { $0.name == "Santa Storyteller" }))
        XCTAssertTrue(presets.contains(where: { $0.name == "Energetic YouTuber" }))
    }
    ...
    func testCharacterPresets() {
        ...
        XCTAssertTrue(characterPresets.contains(where: { $0.name == "Santa Storyteller" }))
    }
    ...
    func testIsCloudVoice() {
        let elevenLabsPreset = VoicePreset.santaStoryteller
