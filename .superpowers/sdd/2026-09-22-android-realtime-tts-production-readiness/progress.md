...
## Fix wave re-reviews: both clean
- ReadAloudAI fix re-review (agent a517496683bec9aeb): all 4 findings genuinely fixed. Rate limiter confirmed real (IP-keyed via req.ip with trust-proxy already set, mounted before requireAuth, non-tautological test firing 12 real HTTP requests). isAvailable() fix confirmed real (2x real MockWebServer, all 3 up/down combinations tested). Own independent assessment: rate-limit-not-auth is a defensible stopgap for invite-only, explicitly insufficient for public launch (matches documented scope). No new issues.
- realtime-tts fix re-review (agent a73cb5668df4126c8): all 5 findings addressed. One residual minor note: MAX_CACHED_VOICE_FILES=80 (~4.92GB) leaves less headroom than the comment implies against the real 7.3GB available (only ~2.4GB for temp/runtime overhead) — workable, not a blocker, flagged for future follow-up (consider 60-65 entries or byte-aware eviction). TIGRIS_BUCKET fail-fast remains deliberately deferred (already labeled Minor/optional). No new issues.
